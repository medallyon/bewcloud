...
export default function FilesSidebar({ path, directories, sortBy, sortOrder, view }: FilesSidebarProps) {
  ...
  return (
    <nav aria-label='Folders' class='flex flex-col gap-1'>
      ...
      <a href={hrefFor(TRASH_PATH)} class={`${linkClass} ${path === TRASH_PATH ? activeLinkClass : defaultLinkClass}`}>
        ...
        ? (
          <ol class='mt-2 flex flex-col gap-1 border-t border-slate-700 pt-2'>
            {ancestors.map((ancestor, index) => (
              <li key={ancestor.path} style={{ paddingLeft: `${index * 0.75}rem` }}>
                <a
